...
public final class RayTraceUtils {
    ...
    public static MovingObjectPosition rayTraceTowards(Entity entity, Rotation rotation, double blockReachDistance) {
        Vec3d start = entity.getPositionEyesCustom(1.0F);
        Vec3d direction = RotationUtils.calcVec3dFromRotation(rotation);
        Vec3d end = start.add(
        		new Vec3d(direction.x * blockReachDistance,
                        direction.y * blockReachDistance,
                        direction.z * blockReachDistance)
        );
        return entity.getEntityWorld().rayTraceBlocks(new Vec3(start), new Vec3(end), false, false, true);
    }
}
