...
package com.moonlight.client.baritone.api.utils;
...
public final class RotationUtils {
    ...
    public static Optional<Rotation> reachable(EntityPlayerSP entity, BlockPos pos, double blockReachDistance) {
        ...
        AxisAlignedBB aabb = state.getBlock().getCollisionBoundingBox(entity.getEntityWorld(), pos, state);
        for (Vec3d sideOffset : BLOCK_SIDE_MULTIPLIERS) {
            double xDiff = aabb.minX * sideOffset.x + aabb.maxX * (1 - sideOffset.x);
            double yDiff = aabb.minY * sideOffset.y + aabb.maxY * (1 - sideOffset.y);
            double zDiff = aabb.minZ * sideOffset.z + aabb.maxZ * (1 - sideOffset.z);
            possibleRotation = reachableOffset(entity, pos, new Vec3d(pos).add(new Vec3d(xDiff, yDiff, zDiff)), blockReachDistance);
            if (possibleRotation.isPresent()) {
                return possibleRotation;
            }
        }
    ...
    public static Optional<Rotation> reachableOffset(Entity entity, BlockPos pos, Vec3d offsetPos, double blockReachDistance) {
        Rotation rotation = calcRotationFromVec3d(entity.getPositionEyesCustom(1.0F), offsetPos, new Rotation(entity.rotationYaw, entity.rotationPitch));
        MovingObjectPosition result = RayTraceUtils.rayTraceTowards(entity, rotation, blockReachDistance);
    ...
    public static Optional<Rotation> reachableCenter(Entity entity, BlockPos pos, double blockReachDistance) {
        return reachableOffset(entity, pos, VecUtils.calculateBlockCenter(entity.getEntityWorld(), pos), blockReachDistance);
    }
}
