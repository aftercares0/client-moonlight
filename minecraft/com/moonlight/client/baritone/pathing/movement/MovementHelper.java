...
package com.moonlight.client.baritone.pathing.movement;

import static com.moonlight.client.baritone.pathing.movement.Movement.HORIZONTALS_BUT_ALSO_DOWN____SO_EVERY_DIRECTION_EXCEPT_UP;
...
import com.moonlight.client.baritone.api.utils.*;
import com.moonlight.client.baritone.api.utils.input.Input;
import com.moonlight.client.baritone.pathing.movement.MovementState.MovementTarget;
...
//             plus magma, which is a normal cube but it hurts you (nuh uh, not in 1.8.9)
            return false;
        }
    ...
    static boolean canPlaceAgainst(IBlockState state) {
        ...
        return state.getBlock().isBlockNormalCube();
    }
    ...
    static boolean throwaway(IPlayerContext ctx, boolean select) {
        ...
        for (byte i = 0; i < 9; i++) {
            ...
            if (!Baritone.settings().blackList.get().contains(item.getItem()) && item.getItem() instanceof ItemBlock) {
                if (select) {
    ...
    static PlaceResult attemptToPlaceABlock(MovementState state, IBaritone baritone, BlockPos placeAt, boolean preferDown) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            if (MovementHelper.canPlaceAgainst(ctx, against1)) {
                ...
                MovingObjectPosition res = RayTraceUtils.rayTraceTowards(ctx.player(), place, ctx.playerController().getBlockReachDistance());
