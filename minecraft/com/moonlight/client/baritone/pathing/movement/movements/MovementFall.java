...
public class MovementFall extends Movement {
    ...
    public MovementState updateState(MovementState state) {
        ...
        if (!isWater && willPlaceBucket() && !playerFeet.equals(dest)) {
            if (!InventoryPlayer.isHotbar(ItemUtil.getSlotFor(STACK_BUCKET_WATER)) || ctx.world().provider.doesWaterVaporize()) {
                return state.setStatus(MovementStatus.UNREACHABLE);
            }

            if (ctx.player().posY - dest.getY() < ctx.playerController().getBlockReachDistance() && !ctx.player().onGround) {
                ctx.player().inventory.currentItem = ItemUtil.getSlotFor(STACK_BUCKET_WATER);

                targetRotation = new Rotation(toDest.getYaw(), 90.0F);

                MovingObjectPosition trace = ctx.objectMouseOver();
                if (trace != null && trace.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK && (trace.getBlockPos().equals(dest) || trace.getBlockPos().equals(dest.down()))) {
        ...
        if (playerFeet.equals(dest) && (ctx.player().posY - playerFeet.getY() < 0.094 || isWater)) { // 0.094 because lilypads
            if (isWater) { // only match water, not flowing water (which we cannot pick up with a bucket)
                if (InventoryPlayer.isHotbar(ItemUtil.getSlotFor(STACK_BUCKET_EMPTY))) {
                    ctx.player().inventory.currentItem = ItemUtil.getSlotFor(STACK_BUCKET_EMPTY);
