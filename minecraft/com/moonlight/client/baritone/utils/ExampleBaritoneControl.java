...
import com.moonlight.client.baritone.api.cache.IRememberedInventory;
import com.moonlight.client.baritone.api.cache.IWaypoint;
import com.moonlight.client.baritone.api.event.events.ChatEvent;
import com.moonlight.client.baritone.api.pathing.goals.*;
import com.moonlight.client.baritone.api.pathing.movement.ActionCosts;
import com.moonlight.client.baritone.api.utils.BetterBlockPos;
import com.moonlight.client.baritone.api.utils.SettingsUtil;
import com.moonlight.client.baritone.behavior.Behavior;
import com.moonlight.client.baritone.behavior.PathingBehavior;
import com.moonlight.client.baritone.cache.ChunkPacker;
import com.moonlight.client.baritone.cache.Waypoint;
...
import com.moonlight.client.baritone.pathing.movement.Moves;
import com.moonlight.client.baritone.process.CustomGoalProcess;
import com.moonlight.client.baritone.utils.pathing.SegmentedCalculator;
...
public class ExampleBaritoneControl extends Behavior implements Helper {
    ...
    public boolean runCommand(String msg0) { // you may think this can be private, but impcat calls it from .b =)
        ...
        if (msg.equals("repack") || msg.equals("rescan")) {
            ...
            for (int x = playerChunkX - 40; x <= playerChunkX + 40; x++) {
                for (int z = playerChunkZ - 40; z <= playerChunkZ + 40; z++) {
                    Chunk chunk = cli.provideChunk(x, z);
                    if (chunk != null) {
                        count++;
        ...
        if (msg.startsWith("thisway") || msg.startsWith("forward")) {
            try {
                Goal goal = GoalXZ.fromDirection(new Vec3(ctx.playerFeetAsVec()), ctx.player().rotationYaw, Double.parseDouble(msg.substring(7).trim()));
                customGoalProcess.setGoal(goal);
                logDirect("Goal: " + goal);
    ...
    private void log(List<ItemStack> stacks) {
        for (ItemStack stack : stacks) {
            if (stack != null) {
                logDirect(stack.getStackSize() + "x " + stack.getDisplayName() + "@" + stack.getItemDamage());
            }
        }
    }
