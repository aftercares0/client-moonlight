...
public class LoginButton {
	...
	public LoginButton(String name, int x, int y, int width, int height) {
		this.name = name;
		this.x = x;
		this.y = y;
		this.width = width;
		this.height = height;
		animate = new ColorAnimate(new Color(255,255,255,120), new Color(255,255,255), 0);
	}
	...
	public void drawButton(int mouseX, int mouseY) {
		...
		RenderUtil.drawRoundedRect(x,
				y, width, height, 5, animate.getColor().getRGB(), 0);
		...
		Fonts.productSans18.drawString(name, x + ((width / 2) - (fontWidth / 2)), y + ((height / 2) - (fontHeight / 2)), Client.MAIN_COLOR.getRGB());
	}
