...
public class NumberComponent extends SettingComponent {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        RenderUtil.drawRect(x + 5, y + 2, width - 10, height - 4, color.darker().getRGB());
        RenderUtil.drawRect(x + 5, y + 2, (int) ((width - 10) * (percentage)), height - 4, color.brighter().getRGB());

        DecimalFormat decimalFormat = new DecimalFormat("#.##");

        String s = value.getName() + ": " + (value.getValue() instanceof Integer ? value.getValue() : decimalFormat.format(value.getValue()));
        
        Fonts.productSans18.drawString(s, x + 10, (int) (y + (height / 2)
