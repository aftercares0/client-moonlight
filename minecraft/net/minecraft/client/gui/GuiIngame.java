...
import com.moonlight.client.animation.fake.MoveAnimate;
...
public class GuiIngame extends Gui
{
    ...
    public MoveAnimate animateTooltip;
    ...
    protected void renderTooltip(ScaledResolution sr, float partialTicks)
    {
    	...
    	if(animateTooltip == null) {
        	animateTooltip = new MoveAnimate(lastLocationX, sr.getScaledHeight() - 28, middleScreen - 92 + entityPlayer.inventory.currentItem * 20 + 1, sr.getScaledHeight() - 28, 1);
        	animateTooltip.updateLastValue(true);
    	}
    	
    	animateTooltip.setX1(lastLocationX);
    	animateTooltip.setX2(middleScreen - 92 + entityPlayer.inventory.currentItem * 20 + 1);

    	if(lastLocationX == 0) {
    		lastLocationX = middleScreen - 92 + entityPlayer.inventory.currentItem * 20 + 1;
    	}
		
    	if(animateTooltip.getLocation()[0] == middleScreen - 92 + entityPlayer.inventory.currentItem * 20 + 1) {
    		lastLocationX = middleScreen - 92 + entityPlayer.inventory.currentItem * 20 + 1;
    	}
    	
    	animateTooltip.update();
    	
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer)
        {
        	GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
        	RenderUtil.drawRoundedRect(middleScreen - 91, sr.getScaledHeight() - 28, 180, 22 , 5 , new Color(0,0,0,90).getRGB(), 0);
        	RenderUtil.drawRoundedRect(animateTooltip.getLocation()[0], sr.getScaledHeight() - 28, 
        			20, 22, 5 ,new Color(0,0,0,100).getRGB(), 0);
            ...
            for(int j = 0; j < 9; ++j) {
                int k = sr.getScaledWidth() / 2 - 90 + j * 20 + 1;
                int l = sr.getScaledHeight() - 25;
...
//            this.getFontRenderer().drawString(s, l1, i1, k1);
    ...
    private void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes)
    {
        ...
        for (Score score1 : collection)
        {
            ...
            String pure = s1.replace("§a", "").replace("§7", "").replace("§r", "").replace("§6", "").replace("§0", "");
            if(pure.contains(".com") || pure.contains(".play") || pure.contains(".net") || pure.contains(".store")
            		|| pure.contains(".shop") || pure.contains(".vip") || pure.contains(".mc")) {
            	s1 = "moonlight";
            }
            ...
            if (j == collection.size())
            {
                ...
                String pure1 = s3.replace("§a", "").replace("§7", "").replace("§r", "").replace("§6", "").replace("§0", "");
                if(pure.contains(".com") || pure.contains(".play") || pure.contains(".net") || pure.contains(".store")
                		|| pure.contains(".shop") || pure.contains(".vip") || pure.contains(".mc")) {
                	s3 = "moonlight";
                }
